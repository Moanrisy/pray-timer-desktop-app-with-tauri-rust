...
impl PrayerSchedule {
    pub fn new(location: Location, date: Date) -> Result<Self, Error> {
        Ok(Self {
            location,
            // date: today()?,
            date: date,
...
impl PrayerTimes {
    pub fn new(date: Date, location: Location, config: Config) -> Result<Self, Error> {
        // println!("before times.rs");
        // let date = date
        //     .with_hms(0, 0, 0)?
        //     .assume_offset(UtcOffset::current_local_offset()?);

        // let date = date.with_hms(0, 0, 0)?.assume_offset(offset!(UTC));
        let date = date.with_hms(0, 0, 0)?.assume_offset(offset!(+7:00:00));
        // println!("after times.rs");
    ...
    fn longitude_difference(location: Location) -> Result<f32, Error> {
        let local_offset = offset!(+7:00:00);
    ...
    pub fn time_remaining(&self, now: OffsetDateTime) -> Result<(u32, u32), Error> {
        let next_prayer_time = self.time(self.next(now)?);
    ...
    /// Get next prayer
    pub fn next(&self, time_now: OffsetDateTime) -> Result<Prayer, Error> {
        match self.current(time_now)? {
    ...
    pub fn current(&self, time_now: OffsetDateTime) -> Result<Prayer, Error> {
        Ok(self.current_time(time_now))
