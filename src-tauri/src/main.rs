...
use islam::pray::{Config, Location, Madhab, Method, PrayerSchedule};
...
fn example() -> Result<AppData, Box<dyn std::error::Error>> {
    ...
    let prayer_times = PrayerSchedule::new(cluring_city)?
        .on(today)
        .with_config(config)
        .calculate()?;
...
#[tauri::command]
fn greet(data: tauri::State<AppData>) -> String {
    format!(
        // "fajr: {}\nsherook: {}\ndohr: {}\nasr: {}\nmaghreb: {}\nishaa: {}\n\nC:{}|N:{}",
        "{} fajr\n{} sherook\n{} dohr\n{} asr\n{} maghreb\n{} ishaa\n\nC:{}|N:{}",
        data.fajr,
...
fn main() {
    let app_data = example().unwrap_or_else(|e| {
